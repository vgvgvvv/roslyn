    ...
    internal sealed class MethodCompiler : CSharpSymbolVisitor<TypeCompilationState, object>
    {
            ...
            return Task.Run(UICultureUtilities.WithCurrentUICulture(() =>
                {
                    try
                    {
                        ...
                        CompileNamespace(symbol);
                        stopwatch.Stop();
                        var time = stopwatch.ElapsedMilliseconds * 1.0f;
                        if (time > 100)
                        {
                            Console.WriteLine($"{symbol.Name} compile cost {time} ms !!");
                        }
                    }
            ...
            TypeCompilationState compilationState)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    else if (methodSymbol.IsScriptInitializer)
                    {
                        ...
                        DefiniteAssignmentPass.Analyze(_compilation, methodSymbol, initializerStatements,
                            unusedDiagnostics, requireOutParamsAssigned: false);
                        DiagnosticsPass.IssueDiagnostics(_compilation, initializerStatements, unusedDiagnostics,
                            methodSymbol);
                    ...
                    else
                    {
                        ...
                        body = BindMethodBody(methodSymbol, compilationState, diagsForCurrentMethod,
                            processedInitializers.AfterInitializersState, out importChain, out originalBodyNested,
                        ...
                        if (includeNonEmptyInitializersInBody && processedInitializers.LoweredInitializers == null)
                        {
                                 ...
                                 methodSymbol is SynthesizedRecordConstructor || _emitTestCoverageData))
                            {
                                if (_emitTestCoverageData && methodSymbol.IsImplicitConstructor)
                                {
                                    ...
                                    DefiniteAssignmentPass.Analyze(_compilation, methodSymbol, analyzedInitializers,
                                        diagsForCurrentMethod, requireOutParamsAssigned: false);
                                }
                            ...
                            else
                            {
                                ...
                                DefiniteAssignmentPass.Analyze(_compilation, methodSymbol, analyzedInitializers,
                                    diagsForCurrentMethod, requireOutParamsAssigned: false);
                                DiagnosticsPass.IssueDiagnostics(_compilation, analyzedInitializers,
                                    diagsForCurrentMethod, methodSymbol);
                            }
                        }
                    }
                ...
                Debug.Assert(processedInitializers.BoundInitializers.IsDefaultOrEmpty || processedInitializers.FirstImportChain != null);
                    ...
                    if (diagsWritten && !methodSymbol.IsImplicitlyDeclared && _compilation.EventQueue != null)
                    {
                        ...
                        if (body != null &&
                            ...
                            _compilation.SemanticModelProvider is CachingSemanticModelProvider
                                cachingSemanticModelProvider)
                        {
                            ...
                            semanticModelWithCachedBoundNodes =
                                (SyntaxTreeSemanticModel)cachingSemanticModelProvider.GetSemanticModel(
                                    syntax.SyntaxTree, _compilation);
                    ...
                    try
                    {
                        ...
                        {
                            Debug.Assert(!(methodSymbol.IsImplicitInstanceConstructor &&
                            ...
                            else
                            {
                                ...
                                if (includeNonEmptyInitializersInBody)
                                {
                                    ...
                                    {
                                        boundStatements =
                                            boundStatements.Add(processedInitializers.LoweredInitializers);
                                    }
                                }
                            ...
                            if (!(methodSymbol is SynthesizedStaticConstructor cctor) ||
            ...
            finally
            {
                ...
                {
                    Console.WriteLine($"{methodSymbol.MetadataName} took {stopwatch.ElapsedMilliseconds}ms to compile.");
                }
            }
        }
